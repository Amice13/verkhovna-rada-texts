...
const START_URL = 'https://komzakonpr.rada.gov.ua'
const TRANSCRIPT_URL = `${START_URL}/news/zacid_komit/stenogr_z`

const NAME = 'Комітет з питань правоохоронної діяльності'
const TARGET_PATH = ['.', 'results', 'komzakonpr']
  ...
  const linksSource = [...htmlPages].map(el => {
    return {
      ...
      title: el.textContent?.trim()
        .replace(/ +/g, ' ')
        .replace('Комітету з питань організації державної влади, місцевого самоврядування, регіонального розвитку та містобудування', '')
        .replace('і Комітету Сейму Литовської Республіки з питань державного управління та місцевого самоврядування', '')
        .replace(', стан виконання покладених на Державне бюро розслідувань завдань та додержання ним законодавства, прав і свобод громадянина', '')
    }
