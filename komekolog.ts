...
const START_URL = 'https://komekolog.rada.gov.ua'
const TRANSCRIPT_URL = `${START_URL}/documents/dijal_komit/Zvity_pro_robotu_Komitetu/transcripts_kom`

const NAME = 'Комітет з питань екологічної політики та природокористування'
const TARGET_PATH = ['.', 'results', 'komekolog']
...
for (const page of pages) {
  ...
  for (const link of links) {
    ...
    const probableHtml = doc.querySelector('.admin-content')
    if ((probableHtml?.children?.length ?? 0) > 0) {
      const content = probableHtml?.innerHTML
