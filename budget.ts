...
const NAME = 'Комітет з питань бюджету'
const START_URL = 'https://budget.rada.gov.ua/news/Diyalnist_Komit/Stenogramy_9skl'
const TARGET_PATH = ['.', 'results', 'budget']
...
for (const page of pages) {
  ...
  for (const link of links) {
    ...
    const title = doc.querySelector('h2')?.textContent?.trim().replace(/\//g, '')
