...
const NAME = 'Комітет з питань соціальної політики та захисту прав ветеранів'
const START_URL = 'https://komspip.rada.gov.ua/news/diyal_kom/stenogr'
const TARGET_PATH = ['.', 'results', 'komspip']
...
for (const page of pages) {
  ...
  for (const link of links) {
    ...
    const title = doc.querySelector('h2')?.textContent?.trim()
      .replace(/\n/g, ' ')
      .replace(/.*?(\d{1,2} [^\d+]+\d{4} року).*/, 'Стенограма від $1')
      .replace(/.*?"([^"]+?)".*/, 'Стенограма від $1')
